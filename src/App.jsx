...
function RoomsScreen({ property, onUpdateProperty, onBack, onScanItem, onViewReport, onNavigate }) {
  ...
  const handleOverrideItem = async (roomId, itemId, overrideVal) => {
    const { error } = await supabase.from("items").update({ override_value: overrideVal }).eq("id", itemId);
    console.log("Override update:", overrideVal, "error:", error);
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#f0f5fb,#e8f1f8)", fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"20px 14px 60px" }}>
        ...
        <div style={{ background:"#f8fafc", border:"1px solid #e2e8f0", borderRadius:14, padding:"14px 16px", marginBottom:14 }}>
          ...
          <div style={{ color:"#374151", fontSize:11, lineHeight:1.7 }}>This report has been generated using AI-assisted image analysis to provide estimated UK new replacement values. All valuations are estimates only and should be reviewed by a qualified insurance professional. Room Worth accepts no liability for any under or over-insurance arising from the use of this report.</div>
        </div>
        ...
        <div style={{ textAlign:"center", padding:"14px 0" }}>
          ...
          <div style={{ color:"#1e293b", fontSize:11, lineHeight:1.6 }}>hello@roomworth.co.uk · www.roomworth.co.uk<br/>Generated {date} · Ref: {reportRef}<br/>© {new Date().getFullYear()} Room Worth</div>
        </div>
      </div>
    </div>
  );
...
function PrivacyPage({ user, onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#f0f5fb,#e8f1f8)", fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif", paddingBottom:120, overflowY:"auto", WebkitOverflowScrolling:"touch" }}>
...
function TermsPage({ onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#f0f5fb,#e8f1f8)", fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif", paddingBottom:40 }}>
      ...
      <div style={{ maxWidth:500, margin:"0 auto", padding:"20px 14px" }}>
        <div style={{ background:"linear-gradient(135deg,#1B3A6B,#1e4d8c)", borderRadius:18, padding:"18px", marginBottom:20 }}>
          <div style={{ color:"white", fontWeight:800, fontSize:16, marginBottom:4 }}>Room Worth</div>
        ...
        <div style={{ background:"white", borderRadius:16, padding:"20px", border:"1px solid #e8eef5", boxShadow:"0 2px 8px rgba(27,58,107,0.05)" }}>
          <S title="1. About Room Worth">Room Worth is a home contents estimation service powered by artificial intelligence, provided by Room Worth to help users estimate the replacement value of their home contents for insurance purposes.</S>
          <S title="2. Important Disclaimer">All valuations are <strong>estimates only</strong> generated by AI analysis. These are not professional insurance valuations. We strongly recommend reviewing all valuations with a qualified valuation expert. Room Worth accepts no liability for any under or over-insurance arising from use of this service.</S>
          ...
          <S title="10. Limitation of Liability">To the fullest extent permitted by law, Room Worth shall not be liable for any indirect or consequential damages arising from use of this service.</S>
        ...
        <div style={{ textAlign:"center", color:"#94a3b8", fontSize:11, marginTop:16 }}>© {new Date().getFullYear()} Room Worth. All rights reserved.</div>
      </div>
    </div>
  );
}
